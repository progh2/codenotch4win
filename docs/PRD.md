...
- 작성일: 2026-09-14 · 최종 갱신: 2026-09-18
- 작성자: progh2 (with Claude)
- 상태: **종결(2026-09-18)** — 업스트림 v1.12.0이 공식 윈도우 인스톨러 배포를 시작하여
  이 포크의 핵심 목적이 달성됨. 저장소는 아카이브.

## 0. 종결 기록 (Post-mortem)

**경과:** 2026-09-14 프로젝트 시작 → 나흘간 v0.4.0(첫 바이너리)·v0.4.1(Cursor Auth0)·
v0.4.2(진단)·v0.4.3(Grok Bot) 릴리스, v0.5.0(자동 업데이트+온보딩+한국어) 코드 완성.
2026-09-16 업스트림 v1.12.0이 자체 윈도우 인스톨러 배포 시작 → 2026-09-18 종결 결정.

**성과:** 업스트림보다 이틀 먼저 윈도우 바이너리를 배포했고, 실사용에서 Cursor Auth0
로그인 버그를 발견·수정했으며, 본가(맥)에도 없는 Grok Bot 주간 버킷 모니터링을 개척.
서명 기반 자동 업데이트 파이프라인, 첫 실행 온보딩, 인스톨러의 구버전 종료 훅,
설정창·호버 카드 한국어화까지 완성 (v0.5.0, 미발행).

**업스트림에 아직 없는 이 포크의 고유 기능** (추후 업스트림 포크에서 기여 후보 —
사전 조사 결과 자동 업데이트는 업스트림에서 논의된 적 없음): ① tauri-plugin-updater
서명 자동 업데이트, ② Grok Bot(`GetSandUsageStatus`), ③ Cursor Auth0 JWT-sub 폴백,
④ 인스톨러 pre-install 구버전 종료, ⑤ 자동 시작 온보딩. PR은 제출하지 않기로 결정
(사용자 판단) — 필요 시 업스트림을 새로 포크해서 진행.
...
| **v0.5.0** | **자동 업데이트**(#4) — tauri-plugin-updater, 시작 시 + 24시간마다 확인, minisign 서명 검증 후 설치·재시작. 포터블은 로그 알림만. 서명 키 시크릿 등록 후 릴리스. **M3 항목 조기 포함:** 첫 실행 시 자동 시작 제안(#8, 1회 질문·거절 존중), 한국어 UI 완성(#9 — 트레이는 기존, 설정 창 97개 문자열 + 호버 카드 TEXT/PATTERNS/UI 테이블 추가), 인스톨러가 실행 중인 구버전 종료 후 설치(#17, NSIS pre-install 훅) |
...
- ✅ 첫 실행 온보딩(v0.5.0): 자동 실행 활성화를 1회 제안 — Yes/No 현지화 대화상자,
  어느 답이든 기억(`autostart_offered`)하고 다시 묻지 않음. 미해결 질문이던
  "제안 vs 기본 켬"은 **제안**으로 결정 (강제 없음, 설정·트레이에서 언제든 변경).
...
- ✅ 확인 주기: 시작 시 + 24시간. 설정 창 "업데이트 확인" 버튼(결과를 문장으로 표시,
  #6)과 포터블 사본의 노치 알림도 v0.5.0에 포함.
...
### FR5 — 한국어 (M3) — v0.5.0에서 조기 구현

- ✅ 트레이 메뉴: 업스트림에 이미 한국어 존재 (`i18n.rs`, 시스템 로케일 자동 감지).
- ✅ 설정 창: `KO_STATIC` 사전(97개 문자열) 추가, 언어 메커니즘을 ru 하드코딩에서
  사전 조회 방식으로 일반화 (settings.html).
- ✅ 호버 카드: 한국어 TEXT/PATTERNS/UI 테이블 추가 (notch.html — 테이블 존재 여부로
  자동 활성화되는 구조).
...
- ~~포터블 zip의 신버전 알림 구현 방식~~ — 해결(v0.5.0): updater 플러그인 check-only +
  노치 notice 이벤트로 표시, 자동 교체 없음.
- ~~온보딩에서 자동 실행 기본값~~ — 해결(v0.5.0): "제안"으로 결정 (1회 질문, 거절 존중).
