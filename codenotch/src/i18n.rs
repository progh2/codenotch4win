...
pub fn tr(lang: &str, key: &str) -> &'static str {
    ...
    match (l, key) {
        ...
        (_, "autostart_offer_title") => "Codenotch",
        ("zh", "autostart_offer") => "在登录 Windows 时自动运行 Codenotch？它会在后台静默等待，直到编码会话开始。之后可以在设置中更改。",
        ("ja", "autostart_offer") => "Windowsサインイン時にCodenotchを自動で開始しますか？コーディングセッションが始まるまでバックグラウンドで待機します。後から設定で変更できます。",
        ("ko", "autostart_offer") => "Windows에 로그인할 때 Codenotch를 자동으로 실행할까요? 코딩 세션이 시작될 때까지 조용히 백그라운드에서 대기합니다. 나중에 설정에서 바꿀 수 있습니다.",
        ("ru", "autostart_offer") => "Запускать Codenotch автоматически при входе в Windows? Он будет тихо ждать в фоне, пока не начнётся сеанс разработки. Это можно изменить в настройках.",
        (_, "autostart_offer") => "Start Codenotch automatically when you sign in to Windows? It waits quietly in the background until a coding session begins. You can change this later in Settings.",
        ("zh", "update_none") => "已是最新版本。",
        ("ja", "update_none") => "最新バージョンです。",
        ("ko", "update_none") => "최신 버전입니다.",
        ("ru", "update_none") => "У вас последняя версия.",
        (_, "update_none") => "You're on the latest version.",
        ("zh", "update_installing") => "正在更新到 {v} — Codenotch 将立即重启",
        ("ja", "update_installing") => "{v} に更新中 — まもなくCodenotchが再起動します",
        ("ko", "update_installing") => "{v}(으)로 업데이트 중 — 잠시 후 Codenotch가 재시작됩니다",
        ("ru", "update_installing") => "Обновление до {v} — Codenotch сейчас перезапустится",
        (_, "update_installing") => "Updating to {v} — Codenotch will restart in a moment",
        ("zh", "update_portable") => "新版本 {v} 已发布 — 请从 Releases 页面下载（便携版不会自动更新）",
        ("ja", "update_portable") => "新しいバージョン {v} が公開されました — Releasesページから入手してください（ポータブル版は自動更新されません）",
        ("ko", "update_portable") => "새 버전 {v}이(가) 나왔습니다 — Releases 페이지에서 받아주세요 (포터블은 자동 업데이트되지 않음)",
        ("ru", "update_portable") => "Доступна новая версия {v} — скачайте её со страницы Releases (переносная версия не обновляется сама)",
        (_, "update_portable") => "{v} is available — download it from the Releases page (the portable copy does not self-update)",
        ("zh", "refresh") => "立即刷新用量",
